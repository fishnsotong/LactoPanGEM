...
def m9(model):
    ...
    #amino acids
    model.reactions.EX_arg_L_e.lower_bound = -100
    model.reactions.EX_cys_L_e.lower_bound = -100
    model.reactions.EX_glu_L_e.lower_bound = -100
    model.reactions.EX_ile_L_e.lower_bound = -100
    model.reactions.EX_leu_L_e.lower_bound = -100
    model.reactions.EX_met_L_e.lower_bound = -100
    model.reactions.EX_tyr_L_e.lower_bound = -100
    model.reactions.EX_phe_L_e.lower_bound = -100
    model.reactions.EX_thr_L_e.lower_bound = -100
    model.reactions.EX_val_L_e.lower_bound = -100
    model.reactions.EX_gly_e.lower_bound = -100
    model.reactions.EX_ala_L_e.lower_bound = -100
    model.reactions.EX_asp_L_e.lower_bound = -100
    model.reactions.EX_his_L_e.lower_bound = -100
    model.reactions.EX_lys_L_e.lower_bound = -100
    model.reactions.EX_pro_L_e.lower_bound = -100
    model.reactions.EX_ser_L_e.lower_bound = -100
    model.reactions.EX_trp_L_e.lower_bound = -100


    #carbon
    model.reactions.EX_glc_D_e.lower_bound = -25.2
    #model.reactions.EX_glc_D_e.upper_bound = -5
    model.reactions.EX_ac_e.lower_bound = -1
    model.reactions.EX_cit_e.lower_bound = -1

    #nuclotides
    model.reactions.EX_thymd_e.lower_bound = -1
    ...
    model.reactions.EX_thm_e.lower_bound = -1
    model.reactions.EX_pydam_e.lower_bound = -1
    model.reactions.EX_pydxn_e.lower_bound = -1
    model.reactions.EX_ribflv_e.lower_bound = -1
    model.reactions.EX_fol_e.lower_bound = -0.1
    model.reactions.EX_ascb_L_e.lower_bound = -1
...
def aux (mod):
    ...
    for x in ex:
        ...
        grc.append(gr)
        a = model.reactions.get_by_id(x).name
        ax.append(a)
    df['exchanges'] = ex
    df[model.id] = grc
    df.set_index('exchanges',inplace=True)
    df=df.T
    dfc.append(df)
    dff = pd.concat(dfc,axis=1)
    return dff   
...
def connectivity(mod):
    model = load_json_model(mod)
    metabolite_connectivity = np.zeros((len(model.metabolites), len(model.metabolites)))

    # Loop through each reaction in the model
    for reaction in model.reactions:
        # Loop through each metabolite participating in the reaction
        for metabolite in reaction.metabolites:
            # Get the index of the metabolite in the matrix
            i = model.metabolites.index(metabolite)
            # Loop through each other metabolite in the reaction and update the matrix
            for other_metabolite in reaction.metabolites:
                j = model.metabolites.index(other_metabolite)
                if i != j:
                    metabolite_connectivity[i, j] += 1

    # Convert the metabolite connectivity matrix to a pandas dataframe
    df = pd.DataFrame(metabolite_connectivity, index=[m.id for m in model.metabolites], columns=[m.id for m in model.metabolites])

    # calculate connectivity for pyruvate and glutamate
    con = pd.DataFrame()
    con['pyr'] = [df['pyr_c'].sum()]
    con['glu'] = [df['glu_L_c'].sum()]
    con['nadh'] = [df['nadh_c'].sum()]
    con['nadph'] = [df['nadph_c'].sum()]
    con['id'] = [model.id]
...
def remove_biomass(model):
    """Removes the BIOMASS reaction from the given model."""
    model.reactions.BIOMASS.remove_from_model()


def remove_unused_metabolites(model):
    """Removes metabolites from the given model that are not present in any reactions."""
    unused_metabolites = [m for m in model.metabolites if len(m.reactions) == 0]
    model.remove_metabolites(unused_metabolites)


def remove_duplicate_genes(model):
    """Removes duplicated genes in the GPR of each reaction in the given model."""
    for reaction in model.reactions:
        if reaction.gene_reaction_rule:
            unique_genes = list(set(reaction.genes))
            reaction.gene_reaction_rule = ' or '.join([f'({g.id})' for g in unique_genes])


def load_and_process_gem(file_path):
    """Loads a GEM from the given JSON file and applies the necessary processing steps."""
    model = load_json_model(file_path)
    remove_biomass(model)
    remove_unused_metabolites(model)
    remove_duplicate_genes(model)
    return model


def save_gem(model, output_dir):
    """Saves the given GEM to a JSON file in the specified output directory."""
    model_id = os.path.splitext(os.path.basename(model.id))[0]
    output_path = os.path.join(output_dir, f'{model_id}.json')
    cobra.io.save_json_model(model,output_path+model.id)


def process_gems(input_dir, output_dir):
    """Loads and processes all GEMs in the input directory, and saves the modified GEMs to the output directory."""
    os.makedirs(output_dir, exist_ok=True)
    file_paths = glob(os.path.join(input_dir, '*.json'))
    with mp.Pool(processes=96) as pool:
        models = pool.map(load_and_process_gem, file_paths)
        for model in models:
            save_gem(model, output_dir)


def count_metabolites(model):
    """Counts the number of metabolites in the given model."""
    return len(model.metabolites)


def count_metabolites_in_gems(input_dir, output_file):
    """Loads and counts the metabolites in all GEMs in the input directory, and saves the results to a CSV file."""
    file_paths = glob(os.path.join(input_dir, '*.json'))
    results = []
    with mp.Pool(processes=96) as pool:
        counts = pool.map(count_metabolites, [load_json_model(fp) for fp in file_paths])
        for fp, c in zip(file_paths, counts):
            model_id = os.path.splitext(os.path.basename(fp))[0]
            results.append({'Model ID': model_id, 'Metabolite Count': c})
    df = pd.DataFrame(results)
    df.to_csv(output_file, index=False)        
        
        
def calculate_robustness_scores(model_path):
    # Load the metabolic model
    model = cobra.io.load_json_model(model_path)

    # Define the biomass reaction and find the set of essential reactions
    biomass_rxn = model.reactions.get_by_id("BIOMASS")
    essential_reactions = cobra.flux_analysis.find_essential_reactions(model)

    # Initialize the reaction robustness scores to 0
    robustness_scores = np.zeros(len(model.reactions))

    # Perform FBA to obtain the reference flux distribution
    model.slim_optimize()

    reference_fluxes = model.solution.x_dict

    # Loop through each reaction in the model and calculate the perturbation-induced change in flux
    for i, reaction in enumerate(model.reactions):
        if reaction not in essential_reactions:
            # Remove the reaction from the model and perform FBA to obtain the new flux distribution
            with model:
                model.reactions.get_by_id(reaction.id).knock_out()
                model.slim_optimize()
                new_fluxes = model.solution.x_dict

            # Calculate the change in flux induced by removing the reaction
            perturbation_fluxes = np.array(list(new_fluxes.values())) - np.array(list(reference_fluxes.values()))

            # Count the number of reactions with a flux change
            num_changed_fluxes = np.count_nonzero(perturbation_fluxes)

            # Calculate the reaction robustness score as the inverse of the number of reactions with a flux change
            robustness_scores[i] = num_changed_fluxes/len(model.reactions) if num_changed_fluxes > 0 else 0.0

    # Normalize the reaction robustness scores between 0 and 1
    robustness_scores = robustness_scores / np.max(robustness_scores)

    # Create a pandas DataFrame with the reaction IDs and their corresponding robustness scores
    df = pd.DataFrame({'reaction_id': [reaction.id for reaction in model.reactions], 'robustness_score': robustness_scores})
